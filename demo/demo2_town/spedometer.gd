...
#	var speed = get_parent().get_parent().get_child(1).get_child(0).linear_velocity.length()
	...
	#	ProjectSettings.set_setting("display/bbaa_paint", !ProjectSettings.get_setting("display/bbaa_paint"));
		ProjectSettings.set_setting("display/bbaa_paint", 1);
	if Input.is_action_just_pressed("SSA_DEBUG_DEPTH"):
	#	ProjectSettings.set_setting("display/depth_paint", !ProjectSettings.get_setting("display/depth_paint"));
